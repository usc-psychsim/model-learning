...
from psychsim.pwl import modelKey, KeyedVector
...
def get_policy(agent, states, model=None, horizon=None, selection=None):
    ...
    :param list[KeyedVector] states: the list of states for which to get the agent's policy.
    ...
    for s in states:
        ...
        pi.append(decision[world.getValue(modelKey(agent.name))]['action'])
    return pi
